...
const BoardUpdate = () => {
  ...
  const doSubmit = async () => {
    if (validation()) {
      ...
      // 게시판 업데이트 호출
      try {
        await updateBoard(formData, navigator, boardNo);
        // 성공 시 알림 표시
        await Swal.fire({
          icon: "success",
          title: "수정 성공",
      ...
      {boardData ? (
        <Container>
          ...
          <Two>
            ...
            <TitleInput onChange={handleTitleChange} value={boardTitle} />
            ...
            <ContentTextarea onChange={(e) => setBoardContent(e.target.value)} value={boardContent} />
            ...
            <FormControl>
              <h5>
                <FormLabel style={{ color: "black" }}>
                  이미지 변경 여부 <span style={{ color: "red" }}>*</span>
                </FormLabel>
              </h5>
              <Error>{error.img}</Error>
              <RadioGroup name="radio-buttons-group" onChange={(e) => setChangeImg(e.target.value)}>
